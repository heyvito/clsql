(ns clsql.require-query-test
  (:require [clojure.test :refer :all]
            ...
            [clsql.helpers :refer [with-databaseless-config]]))
...
(deftest test-running
  (testing "execution with refer"
    (with-databaseless-config
      (let [raw-result (select-twenty-four)
            {:keys [test_result]} raw-result]
        (is (= 24 test_result)))
      (let [raw-result (query-with-placeholder :number 27)
            {:keys [test_result]} raw-result]
        (is (= 27 test_result)))))

  (testing "execution with as"
    (with-databaseless-config
      (let [raw-result (sample/select-twenty-four)
            {:keys [test_result]} raw-result]
        (is (= 24 test_result)))
      (let [raw-result (placeholder-test/query-with-placeholder :number 27)
            {:keys [test_result]} raw-result]
        (is (= 27 test_result))))))
